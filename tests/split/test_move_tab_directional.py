...
async def test_sequential_multi_move_across_groups(
    ...
    async with app.run_test() as pilot:
        ...
        )  # Windows: extra pause for tab switch
        await app.main_view.action_move_editor_right()
        await pilot.wait_for_scheduled_animations()
        await (
            pilot.wait_for_scheduled_animations()
        )  # Windows: extra pause for move action
...
async def test_move_preserves_unsaved_content(
    ...
    async with app.run_test() as pilot:
        ...
        await (
            pilot.wait_for_scheduled_animations()
        )  # Windows: extra pause for move + split creation
...
async def test_move_duplicate_file_activates_existing(
    ...
    async with app.run_test() as pilot:
        ...
        await (
            pilot.wait_for_scheduled_animations()
        )  # Windows: extra pause for split view creation
        ...
        await (
            pilot.wait_for_scheduled_animations()
        )  # Windows: extra pause for tab activation in split
        ...
        await (
            pilot.wait_for_scheduled_animations()
        )  # Windows: extra pause for split + tab deduplication
