...
reads, and circular reference detection.
"""
...
def grammars_dir(tmp_path: Path) -> Path:
    """Create a temporary grammars directory with test .scm files."""
    d = tmp_path / "grammars"
    d.mkdir()
...
def test_resolve_scm_file(grammars_dir: Path):
    ...
    result = _resolve_highlight_query(
        "lang_a",
...
def test_composition_json_includes_all_custom_languages():
    ...
    for lang in _CUSTOM_GRAMMAR_NAMES:
        assert lang in _GRAMMAR_COMPOSITION, (
...
def test_composed_typescript_contains_js_keywords():
    ...
    query = _resolve_highlight_query("typescript")
...
def test_composed_tsx_contains_jsx_and_js():
    ...
    query = _resolve_highlight_query("tsx")
    assert '"function"' in query
...
def test_composed_cpp_contains_c_keywords():
    ...
    query = _resolve_highlight_query("cpp")
...
def test_standalone_language_resolves_via_composition():
    ...
    query = _resolve_highlight_query("dockerfile")
    expected = (_GRAMMARS_DIR / "dockerfile.scm").read_text(encoding="utf-8")
    assert query == expected


def test_composed_queries_are_valid_treesitter():
