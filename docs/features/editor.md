...
- **Command palette**: "Open File..." enumerates workspace files and fuzzy-matches on relative paths. Hidden files (dot-prefixed) are included or excluded based on the `show_hidden_files` setting; `.git/` contents are always excluded. By default, `.gitignore` patterns are respected (powered by `ripgrep-rs`), so files in `.venv/`, `node_modules/`, `build/`, etc. are excluded from results. A **Gitignore toggle checkbox** in the search bar allows users to include gitignored files when needed.
